...
_Rx2.default.Observable.fromEvent(button, 'click').subscribe(function () {
...
var search$ = Observable.fromEvent(input, 'input').map(function (e) {
...
.switchMap(function (term) {
    return Observable.fromPromise(wikiIt(term));
...
function render(result) {
    document.querySelector('#result').innerHTML = result[1].map(function (r) {
        return '<li>' + r + '</li>';
    }).join('');
}
...
var down$ = _Rx2.default.Observable.fromEvent(canvas, 'mousedown').map(function () {
    return 'down';
});
var up$ = _Rx2.default.Observable.fromEvent(canvas, 'mouseup').map(function () {
    return 'up';
});
var upAndDown$ = up$.merge(down$);

var move$ = _Rx2.default.Observable.fromEvent(canvas, 'mousemove').map(function (e) {
...
// 画出连续的线
var diff$ = move$.zip(move$.skip(1), function (first, sec) {
    return [first, sec];
});

upAndDown$.switchMap(function (action) {
    return action === 'down' ? move$ : _Rx2.default.Observable.empty();
}).subscribe(draw);
